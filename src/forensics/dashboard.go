...
func parseLogFile(report *dashboardReport, opts dashboardOptions) {
	...
	finalizeCacheStats(report, cacheHits, cacheMisses, allCachedDurations, mixedDurations, coldDurations)
...
func finalizeCacheStats(report *dashboardReport, hits, misses uint64, allCached, mixed, cold []int64) {
	total := hits + misses
	if total > 0 {
...
<div class="grid">
  ...
  <div class="card"><div class="label">Rate Limits</div><div class="value">%d</div></div>
		...
		r.CacheStats.HitRate, r.CacheStats.UpstreamErrors, r.CacheStats.UpstreamRateLimits,
	)
...
func renderRecentRequestsTable(entries []recentRequestEntry) string {
	...
	out += `<table id="recent-table"><tr><th>Time</th><th>Method</th><th>Route</th><th>Status</th><th>Cache</th><th>Duration</th><th>Hits</th><th>Misses</th><th>Redis</th><th>HTTP</th><th>Mongo</th><th>Request ID</th></tr>`
	for i := len(entries) - 1; i >= 0; i-- {
		e := entries[i]
		out += fmt.Sprintf(`<tr><td>%s</td><td>%s</td><td>%s</td><td>%d</td><td>%s</td><td>%d ms</td><td>%d</td><td>%d</td><td>%d / %.1f ms</td><td>%d / %.1f ms</td><td>%d / %.1f ms</td><td style="font-size:11px;color:#8b949e">%s</td></tr>`,
			esc(e.Timestamp), esc(e.Method), esc(e.Route), e.StatusCode, cacheBadge(e.CacheStatus), e.DurationMs, e.CacheHits, e.CacheMisses, e.RedisCalls, e.RedisMs, e.HTTPCalls, e.HTTPMs, e.MongoCalls, e.MongoMs, esc(e.RequestID))
	}
	return out + `</table>`
}
