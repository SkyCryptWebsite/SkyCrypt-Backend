...
type BlacklistedPath struct {
	isBlacklisted func(path string) bool
}

var BLACKLISTED_PATHS = map[string]BlacklistedPath{
	"HYPIXEL_PLUS": {
		isBlacklisted: func(path string) bool {
...
type BlacklistedParent struct {
	isBlacklisted func(parent string) bool
}

var BLACKLISTED_PARENTS = map[string]BlacklistedParent{
	"HYPIXEL_PLUS": {
		isBlacklisted: func(parent string) bool {
...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: go run tools/formatResourcePackTextures.go <input_folder>")
		return
	}
